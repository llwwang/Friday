...
(async () => {
  ...
  try {
    ...
    if (usernameInput && passwordInput) {
      ...
      // 尝试子账号格式：[YOUR_OWNER_UIN]:[YOUR_USERNAME] 或 [YOUR_USERNAME]@[YOUR_OWNER_UIN]
      console.log('尝试格式1: [YOUR_OWNER_UIN]:[YOUR_USERNAME]');
      ...
      if (currentUrl.includes('login') || currentUrl === 'https://tbds.tbds.boc.fsphere.cn/') {
        ...
        if (usernameInput2 && passwordInput2) {
          console.log('尝试格式2: [YOUR_USERNAME] (纯子账号名)');
          await usernameInput2.fill('[YOUR_USERNAME]');
          await passwordInput2.fill('[YOUR_PASSWORD]');
